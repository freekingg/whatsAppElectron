...
export default {
  ...
  created() {
    // 监听检测结果
    ipcRenderer.on('site-send-message-to-renderer', (event, data) => {
    ...
    // 监听任务完成
    ipcRenderer.on('site-all-finish', (event, data) => {
      this.loading = false
    ...
    // 监听错误
    ipcRenderer.on('site-error', (event, data) => {
      this.$message.error(data)
    })

    // 监听日志
    ipcRenderer.on('site-log', (event, data) => {
      this.log = data
    })
  },
  methods: {

const data = [
  {
    title: '热门推荐',
    list: [
      {
        title: 'chinaz',
        link: 'http://seo.chinaz.com/',
        info: 'seo综合查询，在各大搜索引擎的信息',
      },
      {
        title: '爱站网',
        link: 'https://www.aizhan.com/',
        info: 'seo综合查询，在各大搜索引擎的信息',
      },
      {
        title: '5118',
        link: 'https://www.5118.com/',
        info: '营销大数据 - 智能原创改写,长尾词挖掘,营销情报,站长工具',
      },
      {
        title: '2898',
        link: 'http://www.2898.com/',
        info: '站长资源平台-站长工具-友情链接-免费资源互换-网站交易平台',
      },
      {
        title: '桔子SEO',
        link: 'https://seo.juziseo.com/',
        info: 'seo综合查询，快照历史，反链,外链,老域名,快照历史,站长查询工具',
      },
      {
        title: '百度指数',
        link: 'https://index.baidu.com/v2/index.html#/',
        info: '百度指数是以百度海量网民行为数据为基础的数据分享平台。',
      },
      {
        title: '百度资源平台',
        link: 'https://ziyuan.baidu.com/',
        info: '为开发者、内容创作者、站点管理者等伙伴，提供优化工具、数据、课程、Q&A等服务',
      },
      {
        title: '域名解释',
        link: 'https://www.cxw.com/tool/detail',
        info: '域名解释，企业终端查询，域名匹配终端企业',
      },
      {
        title: '测速-17ce',
        link: 'https://www.17ce.com/',
        info: '国内最专业最权威的实时网站测速',
      },
      {
        title: '聚名网',
        link: 'http://www.juming.com/',
        info: '老域名,域名,域名查询,域名抢注,域名买卖,域名交易,域名管理,聚名网,juming.com',
      },
      {
        title: 'A5交易网',
        link: 'https://www.a5.net/',
        info: '互联网资源交易平台，提供域名网站、新媒体、商标、网店等交易服务。',
      },
      {
        title: '互站网',
        link: 'https://www.huzhan.com/',
        info: '国内知名的网站交易、源码交易、域名交易服务中心',
      },
      {
        title: '爱名网',
        link: 'https://www.22.cn/',
        info: '老域名,域名,域名查询,域名抢注,域名买卖,域名交易,域名管理',
      },
      {
        title: 'godaddy',
        link: 'https://sg.godaddy.com/zh',
        info: '域名注册购买',
      },
      {
        title: '火车头采集',
        link: 'http://www.locoy.com/',
        info:
          '火车采集器软件是一款网页抓取工具，是用于网站信息采集，网站信息抓取，包括图片、文字等信息采集处理发布，是目前使用人数最多的互联网数据采集软件。',
      },
      {
        title: 'DNS检测',
        link: 'https://www.dns.com/health_monitor.html',
        info:
          '通过对网站进行DNS劫持、网页内容劫持、解析速度等全方位的检测，定时输出网站检测报告，提前做好网站防护工作，有效预防以及规避网站被劫持风险。',
      },
      {
        title: '搜外SEO平台',
        link: 'https://didi.seowhy.com/',
        info: '搜外友链平台，智能友链管理专家，专注友情链接交换、交易、查询、监控。',
      },
    ],
  },
  {
    title: 'SEO',
    list: [
      {
        title: 'chinaz工具',
        link: 'http://seo.chinaz.com/',
        info: 'seo综合查询，在各大搜索引擎的信息',
      },
      {
        title: '爱站网',
        link: 'https://www.aizhan.com/',
        info: 'seo综合查询，在各大搜索引擎的信息',
      },
      {
        title: '5118',
        link: 'https://www.5118.com/',
        info: '营销大数据 - 智能原创改写,长尾词挖掘,营销情报,站长工具',
      },
      {
        title: '搜外SEO平台',
        link: 'https://didi.seowhy.com/',
        info: '搜外友链平台，智能友链管理专家，专注友情链接交换、交易、查询、监控。',
      },
      {
        title: '2898',
        link: 'http://www.2898.com/',
        info: '站长资源平台-站长工具-友情链接-免费资源互换-网站交易平台',
      },
      {
        title: '桔子SEO',
        link: 'https://seo.juziseo.com/',
        info: 'seo综合查询，快照历史，反链,外链,老域名,快照历史,站长查询工具',
      },
      {
        title: '百度指数',
        link: 'https://index.baidu.com/v2/index.html#/',
        info: '百度指数是以百度海量网民行为数据为基础的数据分享平台。',
      },
      {
        title: '百度资源平台',
        link: 'https://ziyuan.baidu.com/',
        info: '为开发者、内容创作者、站点管理者等伙伴，提供优化工具、数据、课程、Q&A等服务',
      },
      {
        title: '互站网',
        link: 'https://www.huzhan.com/',
        info: '国内知名的网站交易、源码交易、域名交易服务中心',
      },
      {
        title: '松松站长工具大全',
        link: 'https://tool.lusongsong.com/',
        info: '松松站长工具大全-最全的站长SEO工具导航',
      },
      {
        title: '金花站长工具',
        link: 'http://tool.1n11.com/',
        info: 'EO网站体检软件、网站管理工具、百度360关键词排名查询、移动/外贸网站排名查询、360权重查询',
      },
    ],
  },
  {
    title: '统计',
    list: [
      {
        title: '友盟统计',
        link: 'https://www.umeng.com/',
        info: '360站长平台-站长平台',
      },
      {
        title: '百度统计',
        link: 'https://tongji.baidu.com/web/welcome/login',
        info: '百度统计——领先的中文网站分析平台',
      },
      {
        title: '我要啦51统计',
        link: 'https://web.51.la/',
        info: '我要啦 统计面向网站站长提供免费的、功能完善的、人性化的网站流量统计分析服务',
      },
      {
        title: 'Google站长平台',
        link: 'https://zhanzhang.sm.cn/',
        info: '跟踪您的网站在搜索结果中的排名，并可以浏览 Google 网站站长支持、学习和社区资源',
      },
      {
        title: '神马站长平台',
        link: 'https://zhanzhang.sm.cn/',
        info: '神马站长平台，站长平台，服务合作，开发者，共享，技术，站长',
      },
    ],
  },
  {
    title: '平台',
    list: [
      {
        title: '百度资源平台',
        link: 'https://ziyuan.baidu.com/',
        info: '为开发者、内容创作者、站点管理者等伙伴，提供优化工具、数据、课程、Q&A等服务',
      },
      {
        title: '360站长平台',
        link: 'https://zhanzhang.so.com/',
        info: '360站长平台-站长平台',
      },
      {
        title: '搜狗站长平台',
        link: 'https://zhanzhang.sogou.com/',
        info: '搜狗资源平台_公平开放的交流平台',
      },
      {
        title: 'Google站长平台',
        link: 'https://zhanzhang.sm.cn/',
        info: '跟踪您的网站在搜索结果中的排名，并可以浏览 Google 网站站长支持、学习和社区资源',
      },
      {
        title: '神马站长平台',
        link: 'https://zhanzhang.sm.cn/',
        info: '神马站长平台，站长平台，服务合作，开发者，共享，技术，站长',
      },
    ],
  },
  {
    title: '外链',
    list: [
      {
        title: '松松超级SEO外链工具',
        link: 'https://tool.lusongsong.com/seo/',
        info:
          '网站自动化宣传机器(超级外链工具)可以轻轻松松提高网站曝光率,可以批量增加外链,快速增高世界排名，全部实现自动化外链群发，全部免费，无需人工操作，网络推广必备工具。',
      },
      {
        title: '外链工具',
        link: 'https://ws234.com/seotool/',
        info:
          '网站自动化宣传机器(超级外链工具)可以轻轻松松提高网站曝光率,可以批量增加外链,快速增高世界排名，全部实现自动化外链群发，全部免费，无需人工操作，网络推广必备工具。',
      },
      {
        title: '外链发布工具',
        link: 'http://www.cjzzc.com/wailian.html',
        info:
          '超级外链发布工具收集了数千个网站网址，其中包括IP查询、SEO综合信息查询、SEO外链数量查询、Alexa排名查询以及PR值查询等等网站。',
      },
      {
        title: '老铁外链',
        link: 'http://www.6ke.com.cn/tool/',
        info:
          '1、老铁SEO外链工具原理：就是把您的网址提交大站长工具类似的网站上面进行搜索,然后就会在上面留下痕迹自动生成以网址为标题的静态页面.2、老铁SEO外链发布效果：我们就是利用第三方平台,各种检查查询工具留下您的网址链接,大多数都是网址文本外链,经常来发布外链效果也是不错的.3、使用老铁SEO外链工具',
      },
      {
        title: 'chinaz外链查询',
        link: 'https://outlink.chinaz.com/',
        info: '查询网站在哪些地方做了外链',
      },
      {
        title: '爱站外链查询',
        link: 'https://link.aizhan.com/',
        info: '爱站网为站长提供外链查询结果，帮助您分析网站的首页反链概况，帮助网站健康发展',
      },
    ],
  },
  {
    title: '排名',
    list: [
      {
        title: '百度关键词排名查询',
        link: 'http://tool.chinaz.com/KeyWords/',
        info: '"通过关键词排名查询，可以快速得到当前网站的关键字在Baidu/Google的排名情况。',
      },
      {
        title: '搜狗权重查询',
        link: 'https://sogourank.aizhan.com/',
        info: '搜狗权重,搜狗权重查询,搜狗排名,搜狗排名查询,搜狗流量分析,搜狗流量查询',
      },
      {
        title: '网站排行榜',
        link: 'https://top.chinaz.com/',
        info:
          '网站排行榜是站长之家旗下专业提供中文网站排名服务的栏目，收集了国内各行业排名前列的众多知名网站，是国内专业、领先的中文网站排行榜。',
      },
    ],
  },
  {
    title: '检测',
    list: [
      {
        title: '百度网址检测',
        link: 'https://bsb.baidu.com/',
        info:
          '基于百度大数据和人工智能技术，研发针对恶意网页的检测算法，覆盖欺诈、违法、网页风险类型， 应对黑产各种对抗手段，使恶意信息无处遁形',
      },
      {
        title: '360网址检测',
        link: 'https://webscan.360.cn/',
        info: '360Webscan网站安全检测 | 漏洞扫描工具_网站漏洞修复_网站后门检测',
      },
      {
        title: '腾讯网址检测',
        link: 'https://guanjia.qq.com/online_server/webindex.html',
        info: '网络安全_网址安全检测_网站安全检测诊断工具_域名检测-腾讯电脑管家官网',
      },
      {
        title: 'HTTPS检测工具',
        link: 'http://stool.chinaz.com/https',
        info: '检测部署SSL/TLS的服务是否符合行业最佳实践，PCI DSS支付卡行业安全标准，Apple ATS规范。',
      },
      {
        title: '网站安全检测',
        link: 'https://tools.aizhan.com/webscan/',
        info: '爱站网提供网站安全检测工具，为站长免费提供了网站漏洞检测、网站后门检测、木马查杀、网址安全查询等服务',
      },
      {
        title: '域名风险提示批量检测',
        link: 'http://www.link114.cn/',
        info:
          '批量查询网站收录,批量查询网站PR,批量查询网站IP地址,批量查询网站快照日期,批量查询网站24小时收录量,专为买链接朋友设计的在线查询工具,最好用的站长工具！',
      },
    ],
  },
  {
    title: '分析',
    list: [
      {
        title: '蜘蛛日志分析',
        link: 'https://loghao.com/rizhi/',
        info:
          'LogHao站长工具(www.LogHao.com)是在线网站蜘蛛日志分析工具,选择log/txt日志文件了解百度搜索引擎蜘蛛来路多少及爬取路径;对网站优化有帮助的在线百度蜘蛛在线分析工具.',
      },
      {
        title: '关键词优化分析',
        link: 'http://tool.chinaz.com/kwevaluate',
        info: '关键词优化分析',
      },
    ],
  },
  {
    title: '测速',
    list: [
      {
        title: '17CE',
        link: 'https://www.17ce.com/',
        info:
          'LogHao站长工具(www.LogHao.com)是在线网站蜘蛛日志分析工具,选择log/txt日志文件了解百度搜索引擎蜘蛛来路多少及爬取路径;对网站优化有帮助的在线百度蜘蛛在线分析工具.',
      },
      {
        title: '关键词优化分析',
        link: 'http://ping.chinaz.com/',
        info: '关键词优化分析',
      },
      {
        title: '测速网',
        link: 'https://www.speedtest.cn/',
        info: '测速网 - 专业网速测试, 宽带提速, 游戏测速, 直播测速, 5G测速, 物联网监测 - SpeedTest.cn',
      },
      {
        title: '关键词优化分析',
        link: 'http://ping.chinaz.com/',
        info: '关键词优化分析',
      },
    ],
  },
]
export default data
